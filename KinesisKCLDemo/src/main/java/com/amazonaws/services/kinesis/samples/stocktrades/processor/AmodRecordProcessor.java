 ...
 *
 */
public class AmodRecordProcessor implements IRecordProcessor {

	private static final Log LOG = LogFactory.getLog(AmodRecordProcessor.class);
	...
	private void processRecord(Record record) {
		...
		LOG.info("Partiion Key " + partKey);
		// get data

		ByteBuffer bBuffer = record.getData();
		
		String s = StandardCharsets.UTF_8.decode(bBuffer).toString();

		LOG.info("data is :" + s);
		// get arrival time

		LOG.info("Approximate Arrival Time " + record.getApproximateArrivalTimestamp());
		
		
		String seqNumber = record.getSequenceNumber();
		
		LOG.info("Sequence No" + seqNumber);
	}
